...
- [ ] **Scenarios — war stories**: six ticket-style production problems with
      measured before/after — 3 on a fictional manufacturing-scheduling SaaS
      (ERP import too slow, concurrent Gantt edit conflicts, telemetry-write
      flood) + 3 portfolio mini-projects (deploys eat uploads, API rate-ban +
      retry storm, aggregator goroutine leak). Design doc:
      [`scenarios/README.md`](scenarios/README.md); bullets land in
...
| 2026-07-14 | War-story scenarios added to the plan: after Module 7, six ticket-style production problems (measure baseline → diagnose & fix → measure again), each producing a resume bullet in `../WAR-STORIES.md`. Runs before the capstone. |
| 2026-07-14 | War-story scenarios designed with intent (resume strength + real-world authenticity + concept coverage): 3 manufacturing-SaaS stories that extend Rakshith's real domain profile (ERP import, Gantt edit conflict, telemetry flood) + 3 standalone portfolio mini-projects (thumbnailer drain, API quota + retry storm, aggregator leak). Design doc at `scenarios/README.md`. Scenario 1 (`scenarios/01-erp-import/`) scaffolded: ticket (OPS-1743), DO-NOT-MODIFY fake ERP with a hard 32-connection cap that rejects violations, sequential importer measuring 7.6s baseline vs a <2s target (3.8× miss), measurement harness, SLO-asserting tests (verified failing on the SLO line only; intended fix privately verified at 0.25s then reverted). Module 7 exercises all deferred by choice — gauntlet deprioritized (broad understanding > rehearsed builds), ex4 leak-hunt parked for later. |
| 2026-07-14 | Module 7 (interview gauntlet + internals) opened: notes (hchan's three send paths + the bakery-counter analogy, closed-channel mechanics, G-M-P with work stealing + the two-kinds-of-blocked distinction, pprof leak-hunting recipe, go tool trace), 4 demos (hchan paths live, preemption + 10k parked goroutines measured at ~8KB each, a leaky server to profile for real, a traced unbalanced pipeline), 4 exercises — ex1-ex3 are TIMED gauntlet builds (errgroup, token bucket, singleflight from scratch, 25-min cold attempts), ex4 is a leak hunt where the failing test prints the goroutine profile and the profile names the guilty line. All demos verified running; all tests verified failing for intended reasons only. Stdlib only, nothing new to vendor. |
